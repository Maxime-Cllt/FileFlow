...
mod tests {
    ...
    use crate::fileflow::fileflow::{get_create_statement, get_drop_statement, get_insert_into_statement, get_create_statement_with_fixed_size, get_formated_column_names};
    ...
    async fn test_get_create_statement_with_fixed_size() {
        let snake_case_headers: Vec<String> = vec!["header1".to_string(), "header2".to_string()];
        let map_max_length: HashMap<&str, usize> = snake_case_headers.iter().map(|h| (h.as_str(), 0)).collect();
        let final_table_name: &str = "test_table";

        let mut db_driver: HashMap<&str, &str> = HashMap::new();
        db_driver.insert(POSTGRES, "CREATE TABLE \"test_table\" (\"header1\" VARCHAR(0), \"header2\" VARCHAR(0))");
        db_driver.insert(MYSQL, "CREATE TABLE `test_table` (`header1` VARCHAR(0), `header2` VARCHAR(0))");
        db_driver.insert(MARIADB, "CREATE TABLE `test_table` (`header1` VARCHAR(0), `header2` VARCHAR(0))");
        db_driver.insert(SQLITE, "CREATE TABLE \"test_table\" (\"header1\" VARCHAR(0), \"header2\" VARCHAR(0))");

        for (driver, expected) in db_driver {
            let result: Result<String, String> = get_create_statement_with_fixed_size(driver, final_table_name, &map_max_length, &snake_case_headers);
            assert_eq!(result.unwrap(), expected);
        }

        let map_max_length: HashMap<&str, usize> = snake_case_headers.iter().map(|h| (h.as_str(), 10)).collect();
        let mut db_driver: HashMap<&str, &str> = HashMap::new();
        db_driver.insert(POSTGRES, "CREATE TABLE \"test_table\" (\"header1\" VARCHAR(10), \"header2\" VARCHAR(10))");
        db_driver.insert(MYSQL, "CREATE TABLE `test_table` (`header1` VARCHAR(10), `header2` VARCHAR(10))");
        db_driver.insert(MARIADB, "CREATE TABLE `test_table` (`header1` VARCHAR(10), `header2` VARCHAR(10))");
        db_driver.insert(SQLITE, "CREATE TABLE \"test_table\" (\"header1\" VARCHAR(10), \"header2\" VARCHAR(10))");
    ...
    async fn test_get_formated_column_names() {
        ...
        assert_eq!(formatted_headers, vec!["header_1".to_string(), "_header2".to_string()]);

        let headers: Vec<String> = vec!["header    1".to_string(), "".to_string(), "header2".to_string()];
        let formatted_headers: Vec<String> = get_formated_column_names(headers);
        assert_eq!(formatted_headers, vec!["header____1".to_string(), "column_2".to_string(), "header2".to_string()]);
    }
    ...
    async fn test_generate_load_data_sql() {
        ...
        let file: File = File::create(&csv_file_path).expect("Failed to create CSV file");
        let mut wtr: Writer<File> = Writer::from_writer(file);
        wtr.write_record(&["header1", "header2"]).expect("Failed to write record");
        wtr.write_record(&["value1", "value2"]).expect("Failed to write record");
        wtr.write_record(&["value3", "value4"]).expect("Failed to write record");
        wtr.flush().expect("Failed to flush CSV writer");
        ...
        let result: Result<String, String> = crate::fileflow::action::actions::generate_load_data_sql(&config).await;
        ...
        sql.push_str(get_create_statement_with_fixed_size(MARIADB, final_table_name, &size_map, &snake_case_headers).unwrap().as_str());
        sql.push_str("\n");
